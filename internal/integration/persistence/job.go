...
const jobColumns = `id, name, description, type, working_directory, schedule_enabled, schedule_type, cron_expression,
		...
		max_retries, model, override_repo_command, claude_command, interpreter, script_content, env_variables,
...
func (p *jobPersistence) SaveJob(job entity.Job) error {
	...
	query := `INSERT INTO jobs (id, name, description, type, working_directory, schedule_enabled, schedule_type,
		...
		autonomous_mode, max_retries, model, override_repo_command, claude_command, interpreter, script_content,
		env_variables, created_at, updated_at, last_run_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
func (p *jobPersistence) UpdateJob(job entity.Job) error {
	...
	query := `UPDATE jobs SET name = ?, description = ?, type = ?, working_directory = ?,
		...
		claude_command = ?, interpreter = ?, script_content = ?, env_variables = ?,
	...
	result, err := p.db.Exec(query,
		...
		row.ClaudeCommand, row.Interpreter, row.ScriptContent, row.EnvVariables,
